...
- [Common API Gateway features](#common-api-gateway-features)
  ...
  - [Customizing request body mapping templates](#customizing-request-body-mapping-templates)
    ...
    - [SQS](#sqs-1)
    - [SNS](#sns-1)
  - [Setting API keys and defining usage-plans](#setting-api-keys-and-defining-usage-plans)
...
```
### Setting API keys and defining usage-plans

You can specify a list of API keys to be used by your Rest API by adding an 'apiKeys' array property to the provider object in your serverless.yml file. You'll also have to specify explicitly which endpoints should be protected. This can be done by adding a 'private' boolean property to the 'http' event property under the function handler configuration object.

API Keys are created globally, so if you wish to deploy your service to a different stage make sure that your API key contains a stage variable, as defined below. Note also that, when using API keys, you can optionally define usage plan quota and throttle as needed using a usagePlan configuration object shown below.
...
Please also note that those are the API keys names, not the actual values. Once you deploy your service, the value of those API keys will be auto generated by AWS and printed on the screen for you to use. The values can be concealed from the output with the --conceal deploy option.

Once deployed, your endpoints will be protected and consumers of your API will have to include in their requests an 'x-api-key' header, containing one of the generated keys. This will only be necessary for functions where explicitly set to private.