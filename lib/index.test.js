  ...
  describe('#constructor()', () => {
    ...
    it('should run package:compileEvents in order', async () => {
      ...
      const compileApiKeysStub = sinon
        .stub(serverlessApigatewayServiceProxy, 'compileApiKeys')
        .returns(BbPromise.resolve())
      const compileUsagePlanStub = sinon
        .stub(serverlessApigatewayServiceProxy, 'compileUsagePlan')
        .returns(BbPromise.resolve())
      const compileUsagePlanKeysStub = sinon
        .stub(serverlessApigatewayServiceProxy, 'compileUsagePlanKeys')
        .returns(BbPromise.resolve())
      ...
      await expect(
        ...
        expect(compileApiKeysStub.calledOnce).to.be.equal(true)
        expect(compileUsagePlanStub.calledOnce).to.be.equal(true)
        expect(compileUsagePlanKeysStub.calledOnce).to.be.equal(true)
      ...
      serverlessApigatewayServiceProxy.compileApiKeys.restore()
      serverlessApigatewayServiceProxy.compileUsagePlan.restore()
      serverlessApigatewayServiceProxy.compileUsagePlanKeys.restore()
