...
describe('#compileMethodsToDynamodb()', () => {
  ...
  const testQueryItem = (params, intRequestTemplates, intResponseTemplates) => {
    const http = _.merge(
      {},
      {
        path: 'dynamodb',
  ...
  describe('#query method', () => {
    it('should create corresponding resources when indexname is given', () => {
      const intRequestTemplate = {
        'Fn::Sub': [
          '{"TableName": "${TableName}","IndexName": "${IndexName}","KeyConditionExpression": "#dynamo_${HashKey} = :v1 and #dynamo_${RangeKey} = :v2","ExpressionAttributeValues" : {":v1": {"${HashAttributeType}":"${HashAttributeValue}"},":v2": {"${RangeAttributeType}":"${RangeAttributeValue}"}},"ExpressionAttributeNames" : {"#dynamo_${HashKey}": "${HashKey}", "#dynamo_${RangeKey}": "${RangeKey}"}}',
          {
            TableName: {
              Ref: 'MyTable'
            },
      ...
      const intResponseTemplate =
        '#set($inputRoot = $input.path(\'$\')) [#if($inputRoot.Count > 0)#foreach($item in $inputRoot.Items){#foreach($key in $item.keySet())#set ($value = $item.get($key))#foreach( $type in $value.keySet())"$key":"$value.get($type)"#end#if($foreach.hasNext()) , #end#end} #if($foreach.hasNext()) , #end#end#end]'
      testQueryItem(
        {
          hashKey: { pathParam: 'id', attributeType: 'S' },
          rangeKey: { queryStringParam: 'range', attributeType: 'S' },
