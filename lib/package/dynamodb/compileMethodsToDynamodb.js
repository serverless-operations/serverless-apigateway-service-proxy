...
module.exports = {
  ...
  getDefaultDynamodbResponseTemplates(http) {
    ...
    if (http.action === 'Scan' || http.action === 'Query') {
      return {
        'application/json': this.getScanDefaultDynamodbResponseTemplate(),
        'application/x-www-form-urlencoded': this.getScanDefaultDynamodbResponseTemplate()
      }
    }
  ...
  getScanDefaultDynamodbResponseTemplate() {
    return oneLineTrim`
  ...
  buildDefaultDynamodbRequestTemplate(http) {
    switch (http.action) {
      ...
      case 'Scan':
        return this.buildDefaultDynamodbScanRequestTemplate(http)
      case 'Query':
        return this.buildDefaultDynamodbQueryRequestTemplate(http)
    }
  },
  ...
  buildDefaultDynamodbScanRequestTemplate(http) {
    const fuSubValues = {
      TableName: http.tableName
    }
    const requestTemplate = '{"TableName": "${TableName}"}'
    return {
      'Fn::Sub': [`${requestTemplate}`, fuSubValues]
    }
  },
  buildDefaultDynamodbQueryRequestTemplate(http) {
    const fuSubValues = {
      TableName: http.tableName,
      ColumnName: http.columnName,
      Index: http.index
    }
    const requestTemplate =
      '#set($parameterValue = "$input.params(\'parameter\')")\n{"TableName":"${TableName}","IndexName":"${Index}","KeyConditionExpression":"${ColumnName}=:v1","ExpressionAttributeValues":{":v1":{"S":"$parameterValue"}}}'
    return {
      'Fn::Sub': [`${requestTemplate}`, fuSubValues]
    }
  },
