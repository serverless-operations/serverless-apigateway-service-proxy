...
module.exports = {
  ...
  getDefaultDynamodbResponseTemplates(http) {
    ...
    } else if (http.action === 'Query') {
      return {
        'application/json': this.getQueryItemDefaultDynamodbResponseTemplate(),
  ...
  getQueryItemDefaultDynamodbResponseTemplate() {
    return oneLineTrim`
  ...
  buildDefaultDynamodbRequestTemplate(http) {
    switch (http.action) {
      ...
      case 'Query':
        return this.buildDefaultDynamodbQueryItemRequestTemplate(http)
    }
  },
  ...
  buildDefaultDynamodbQueryItemRequestTemplate(http) {
    const fuSubValues = {
      TableName: http.tableName
    }

    let requestTemplate = '{"TableName": "${TableName}",'
    if (_.has(http, 'indexName')) {
      requestTemplate += '"IndexName": "${IndexName}",'
      Object.assign(fuSubValues, { IndexName: http.indexName })
    }

    let keyConditionExpression = '"KeyConditionExpression": "'
    let expressionAttributeValues = '"ExpressionAttributeValues" : {'
    let expressionAttributeNames = '"ExpressionAttributeNames" : {'

    if (_.has(http, 'hashKey')) {
      if (_.has(http.hashKey, 'queryOperator')) {
        keyConditionExpression += '#dynamo_${HashKey} ${QueryOperatorHashKey} :v1'
        Object.assign(fuSubValues, { QueryOperatorHashKey: http.hashKey.queryOperator })
      } else {
        keyConditionExpression += '#dynamo_${HashKey} = :v1'
      }
      expressionAttributeValues += '":v1": {"${HashAttributeType}":"${HashAttributeValue}"}'
      expressionAttributeNames += '"#dynamo_${HashKey}": "${HashKey}"'
      Object.assign(fuSubValues, this.getDynamodbHashkeyFnSubValues(http))
    }

    if (_.has(http, 'rangeKey')) {
      if (_.has(http.rangeKey, 'queryOperator')) {
        keyConditionExpression += '#dynamo_${RangeKey} ${QueryOperatorRangeKey} :v1'
        Object.assign(fuSubValues, { QueryOperatorRangeKey: http.rangeKey.queryOperator })
      } else {
        keyConditionExpression += ' and #dynamo_${RangeKey} = :v2'
      }
      expressionAttributeValues += ',":v2": {"${RangeAttributeType}":"${RangeAttributeValue}"}'
      expressionAttributeNames += ', "#dynamo_${RangeKey}": "${RangeKey}"'
    ...
    requestTemplate += keyConditionExpression
    requestTemplate += expressionAttributeValues
    requestTemplate += expressionAttributeNames
    requestTemplate += '}'
    return {
      'Fn::Sub': [`${requestTemplate}`, fuSubValues]
    }
